...
//  @Published public var isLoading: Bool = true
  ...
  public var max: Double { ar.reduce(into: -Double.infinity) { r, e in
    r = [e.1, e.2, e.3, e.4, r].max()!
  }
  }
  public var min: Double { ar.reduce(into: +Double.infinity) { r, e in
    r = [e.1, e.2, e.3, e.4, r].min()!
  }
  }
  public var vmax: Double { ar.reduce(into: -Double.infinity) { r, e in
    r = [e.5, r].max()!
  }
  }
  ...
  public var xticks: [xtick] {
    ...
    for (i, e) in ar.enumerated() {
      ...
      st = false; e_d = extractMonth(e.date)
    }
        ...
        Task {
          ...
          do {
            ...
            result = ar.map { subArray in
              let date = subArray[0] as! String, open = subArray[1] as! Double
              let high = subArray[2] as! Double, low = subArray[3] as! Double
...
//    .catch { err in print("catch = \(err)"); fatalError("catch"); Just(-1) }
    ...
    .sink() { print("sink err = \($0)") } receiveValue: { [weak self] in
